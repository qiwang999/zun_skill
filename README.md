...
```mermaid
flowchart TD
    A["📝 用户输入<br>「你为什么一个人做游戏？」"] --> B{"Step 1 · SKILL.md 内联语录"}
    B -->|命中核心语录| Z["✅ 直接返回"]
    B -->|不够？| C["Step 2 · 双向量语义检索"]

    C --> CQ["🔍 提问集合<br>zun_questions<br>语义搜索相似提问"]
    C --> CA["🔍 回答集合<br>zun_quotes<br>直接语义搜索回答"]

    CQ --> CQ_HIT["命中：<br>「不玩自己做的游戏吗？」"]
    CA --> CA_HIT["命中：<br>「为什么我不玩社交游戏」"]

    CQ_HIT --> M["🔗 合并·去重·按相似度排序<br>question_match 优先"]
    CA_HIT --> M

    M --> D{相似度 >= 0.7 ?}
    D -- 是 --> R["🎯 返回匹配结果"]
    D -- 否 / 无覆盖 --> E["Step 3 · 网络检索<br>ZUN {关键词} 発言 訪談"]
    E --> F["❌ 「这个嘛……不太记得了（笑）」"]
```
...
```
zun-persona/
├── SKILL.md                          # 太田顺也.skill 核心文件（激活时加载）
├── README.md                         # 本文件（中文）
