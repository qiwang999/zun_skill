...
```mermaid
flowchart TD
    A["📝 ユーザー入力<br>「なぜ一人でゲームを作るの？」"] --> B{"Step 1 · SKILL.md インライン語録"}
    B -- コア語録にヒット --> Z["✅ 直接返す"]
    B -- 足りない？ --> C["Step 2 · デュアルベクトル検索"]

    C --> CQ["🔍 質問コレクション<br>zun_questions<br>類似質問を検索"]
    C --> CA["🔍 回答コレクション<br>zun_quotes<br>回答を直接検索"]

    CQ --> CQ_HIT["命中：<br>「自分のゲーム遊ばないの？」"]
    CA --> CA_HIT["命中：<br>「なぜソーシャルゲームしない」"]

    CQ_HIT --> M["🔗 統合·重複排除·類似度ランク付け<br>question_match 優先"]
    CA_HIT --> M

    M --> D{類似度 >= 0.7 ?}
    D -- はい --> R["🎯 結果を返す"]
    D -- いいえ / カバーなし --> E["Step 3 · ウェブ検索<br>ZUN {キーワード} 発言 訪談"]
    E --> F["❌ 「うーん……あんまり覚えてないな（笑）」"]
```
...
**語録** は彼が何を言ったかを教える。<br>
**太田顺也.skill** は彼の視点で物事を見る手助けをする。<br><br>
